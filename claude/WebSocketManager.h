    ...
    void handleMessage(const char* message) {
        ...
        // ---- Relay konfiguráció mentés ----
            ...
            for (JsonObject r : relays) {
                ...
                uint8_t     pin       = r["pin"].as<uint8_t>();
                bool        activeLow = r["activeLow"].as<bool>();
                const char* name      = r["name"] | "";
                eeprom->saveRelayConfig(idx, true, pin, activeLow,
                    (uint8_t)DeviceType::GENERIC, (uint8_t)ModuleType::NONE);
                eeprom->saveRelayName(idx + 1, String(name));
                Serial.printf("[WS] Relay %d mentve: pin=%d nev=%s\n", idx + 1, pin, name);
                idx++;
            }

            // Broadcast: minden kliens értesül az újraindulásról
            {
